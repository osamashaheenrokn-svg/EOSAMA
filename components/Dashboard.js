...
function progressColor(pct) {
  ...
  if (pct < 75) return { text: "text-amber-600", stroke: "#e8871e", bg: "bg-amber-50", border: "border-amber-200" };
...
export function Dashboard({ profile, userEmail }) {
  ...
  async function addSubClaim(subId, entry) {
    await insertRow("subcontractor_claims", {
      subcontractor_id: subId, number: Number(entry.number), amount: Number(entry.amount), date: entry.date || null,
      includes_vat: !!entry.includesVat, vat_rate: entry.includesVat ? Number(entry.vatRate || 0) : 0,
    });
      ...
      <div className="bg-slate-900 text-stone-100 px-6 py-4 flex items-center justify-between flex-wrap gap-3">
        <button onClick={() => setView("home")} className="flex items-center gap-4 text-right">
          <svg width="56" height="56" viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">
            ...
            <rect x="0" y="0" width="200" height="200" rx="28" fill="#24272d" />
