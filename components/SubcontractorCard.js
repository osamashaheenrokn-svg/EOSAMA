          ...
          <tbody>
            ...
            {claims.map((c) => (
              <tr key={c.id} className="border-t border-stone-100">
                ...
                <td className="p-1.5"><AttachmentCell path={c.attachment_path} canEdit={canEdit} inputId={`claim-${c.id}`} onUpload={(file) => onAttachClaim(c.id, file)} /></td>
                {canManage && <td className="p-1.5"><button onClick={() => onDeleteClaim(c.id)} className="text-rose-600 border border-rose-200 rounded px-1 py-0.5"><Trash2 className="w-3 h-3" /></button></td>}
              </tr>
            ))}
          </tbody>
        </table>
          ...
          <tbody>
            ...
            {payments.map((c) => (
              <tr key={c.id} className="border-t border-stone-100">
                ...
                <td className="p-1.5"><AttachmentCell path={c.attachment_path} canEdit={canEdit} inputId={`payment-${c.id}`} onUpload={(file) => onAttachPayment(c.id, file)} /></td>
                {canManage && <td className="p-1.5"><button onClick={() => onDeletePayment(c.id)} className="text-rose-600 border border-rose-200 rounded px-1 py-0.5"><Trash2 className="w-3 h-3" /></button></td>}
              </tr>
            ))}
          </tbody>
        </table>
