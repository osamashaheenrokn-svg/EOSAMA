...
// Default salary due for a given 'YYYY-MM': the full monthly_salary, except the
// month the staff member actually started in, which is prorated by days worked
// (so starting on 27/4 only counts ~3 days of April, not a full month).
export function proratedSalaryForMonth(member, month) {
  const salary = Number(member.monthly_salary || 0);
  if (!member.start_date) return salary;
  const [y, m] = month.split("-").map(Number);
  const start = new Date(member.start_date);
  if (start.getFullYear() !== y || start.getMonth() + 1 !== m) return salary;
...
// A subcontractor claim's amount is entered VAT-inclusive when includes_vat is set —
// the VAT riyal amount is derived from the percentage, not stored redundantly, so it
// always matches the current amount even if the claim is edited later.
...
export async function fetchCompanyFinancials(supabase, projects) {
  const [custodyReceived, custodySpent, laborCosts, laborPayments, staff, revenues, subcontractors] = await Promise.all([
    ...
    supabase.from("staff").select("project_id, monthly_salary, start_date, staff_payments(month, amount, overtime)"),
  ...
  for (const s of subcontractors.data || []) {
    ...
    subClaimsNetByProject[s.project_id] = (subClaimsNetByProject[s.project_id] || 0) + sumNetClaims(s.subcontractor_claims);
  ...
  return projects.map((p) => {
    ...
    const subClaimsNet = subClaimsNetByProject[p.id] || 0;
    ...
    const expenses = custodySpentAmt + laborCost + staffPaid + subClaimsNet;
    return {
      id: p.id,
      name: p.name,
      expenses,
