...
export async function fetchQuotations(supabase) {
...
// The current month is never "overdue" — a salary is only late once the month it's
// owed for has fully ended and it still hasn't been registered as paid. So this sums
// every past (completed) month since start_date/end_date that has no payment on file.
export function staffOverdueAmount(member, today = new Date()) {
  const todayStr = today.toISOString().slice(0, 10);
  if (member.start_date && member.start_date > todayStr) return 0;
  if (member.end_date && member.end_date < todayStr) return 0;
  const thisMonth = currentMonthKey(today);
  return monthsSinceStart(member.start_date, member.end_date, today)
    .filter((m) => m !== thisMonth && !paymentForMonth(member, m))
    .reduce((a, m) => a + proratedSalaryForMonth(member, m), 0);
}
...
export function staffStatus(member, today = new Date()) {
  ...
  if (staffOverdueAmount(member, today) > 0) return { label: "متأخر", color: "bg-rose-100 text-rose-800", overdue: true };
  return { label: "لم يحن الموعد بعد", color: "bg-stone-100 text-stone-600", overdue: false };
}
...
export function staffOverdueTotal(staff, today = new Date()) {
  return (staff || []).reduce((a, s) => a + staffOverdueAmount(s, today), 0);
}
...
export async function fetchCompanyFinancials(supabase, projects) {
  ...
  return projects.map((p) => {
    ...
    return {
      ...
      pendingBilling: Number(p.pending_billing || 0),
