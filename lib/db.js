...
// The VAT riyal amount is derived from the claim's amount and percentage, not stored
// redundantly, so it always matches even if the claim amount is edited later.
export function claimVatAmount(claim) {
  if (!claim.includes_vat) return 0;
  const rate = Number(claim.vat_rate || 0);
  return (Number(claim.amount || 0) * rate) / 100;
}
